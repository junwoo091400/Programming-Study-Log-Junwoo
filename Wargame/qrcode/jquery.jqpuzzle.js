...
(function($) {
            ...
            $srcImg.one('load', function() {
                ...
                var shuffle = function(rounds, speed) {
                    if (speed) {
                        ...
                        if (lock)
                            return false;
                        if (control.confirmShuffle && (moves > 0) && !window.confirm(texts.confirmShuffleMessage))
                            return false;
                        lock = true;
                        if (solved) {
                ...
                if (settings.shuffle)
                    shuffle(1);
                var $background = $('<div/>').css({ // Aha, this is the ORIGINAL image, also a BACKGROUND.
                    width: fullWidth,
                    height: fullHeight,
                ...
                if (control.counter || control.timer) {
                    ...
                    if (control.counter)
                        $counter = $protoField.clone().appendTo($display).after(texts.movesLabel + ' ');
                    if (control.timer)
                        $timer = $protoField.clone().appendTo($display).after(texts.secondsLabel);
                    if (!settings.shuffle)
    ...
    ;
    console.log('Puzzle defenition finished');
